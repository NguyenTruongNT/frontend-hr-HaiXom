...
const RegisterShiftModal = ({
  ...
  userType = "part",
  weekRange,
  realShiftTypes = [], // Nhận từ API
  realDays = [], // Nhận từ API
  shiftDemands = {}, // Nhận từ API
  fixedOffShifts = {}, // Nhận từ API
  onSubmit,
}) => {
  // --- STATES & LOGIC ---
  ...
  const handleSendRegistration = () => {
    ...
    setTimeout(() => {
      ...
      onClose();
      setSelectedShifts({}); // Reset sau khi gửi thành công
    }, 2000);
  };
  ...
  const toggleShift = (date, shiftId) => {
    ...
    // Nếu ca này nằm trong danh sách nghỉ cố định từ API thì khóa
    if (fixedOffShifts[date]?.includes(shiftId)) return;
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-end bg-black/60 backdrop-blur-sm transition-all">
      ...
      <div className="h-full w-full max-w-md bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-500 relative">
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-xs text-indigo-500 font-black mt-1 uppercase tracking-widest leading-none">
                {weekRange || "Đang tải tuần làm việc..."}
              </p>
            </div>
            <button
              onClick={onClose}
        ...
        >
          {/* Kiểm tra nếu API chưa trả về dữ liệu ngày tháng */}
          {realDays.length === 0 ? (
            ...
            realDays.map((day) => (
              <div key={day.date} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {realShiftTypes.map((shift) => {
                    const isSelected = selectedShifts[day.date]?.includes(
                      shift.id,
                    );
                    const isFixedOff = fixedOffShifts[day.date]?.includes(
                      shift.id,
                    );
                    ...
                    return (
                      ...
                      >
                        {/* Badge Định mức lấy từ API */}
                        {!isFixedOff &&
                          (() => {
                            // Lấy object demand từ API (ví dụ: { required: 5, registered: 3 })
                            const demand = shiftDemands[day.date]?.[shift.id];
                            const registered = demand?.registered ?? 0;
                            const required = demand?.required ?? 0;
                            const isFull =
                              registered >= required && required > 0;
        ...
        ${
          isSelected
            ...
            : isFull
              ? "bg-rose-100 text-rose-600" // Hiện màu đỏ nhẹ nếu đã đủ người
              : "bg-slate-100 text-slate-500"
        }`}
                              >
                                <Users size={10} />
                                {/* Hiển thị định dạng Đã đăng ký / Cần có */}
                                <span>
                                  {registered}/{required}
                                </span>
                              </div>
                        ...
                        {isFixedOff ? (
                          <>
                            ...
                            <span
                              className={`text-[9px] font-bold mt-1 opacity-60 ${isSelected ? "text-white" : "text-slate-400"}`}
                            >
                              {shift.time}
                            </span>
                          </>
                          ...
                          <>
                            {/* Chấm nhỏ cảnh báo nếu ca đã đủ người mà mình chưa chọn */}
                            {(() => {
                              const demand = shiftDemands[day.date]?.[shift.id];
                              const isFull =
                                (demand?.registered ?? 0) >=
