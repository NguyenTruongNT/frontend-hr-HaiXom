...
const mockDemands = {
  "2026-03-09": { Sáng: { registered: 4, required: 5 }, Chiều: { registered: 2, required: 4 } },
  "2026-03-10": { Sáng: { registered: 5, required: 5 }, Chiều: { registered: 3, required: 4 } },
};
...
const RegisterShiftModal = ({
  ...
  const displayDays = useMemo(() => (realDays && realDays.length > 0 ? realDays : mockDays), [realDays]);
  const displayShiftTypes = useMemo(() => (realShiftTypes && realShiftTypes.length > 0 ? realShiftTypes : mockShiftTypes), [realShiftTypes]);
  const displayDemands = useMemo(() => (shiftDemands && Object.keys(shiftDemands).length > 0 ? shiftDemands : mockDemands), [shiftDemands]);
  const displayFixedOff = useMemo(() => (fixedOffShifts && Object.keys(fixedOffShifts).length > 0 ? fixedOffShifts : mockFixedOff), [fixedOffShifts]);
  ...
  const toggleShift = (date, shiftId) => {
    ...
    const isFull = demand?.registered >= demand?.required && demand?.required > 0;
                    ...
                    >
                      ...
                      {isFixedOff ? (
                        ...
                        <>
                          ...
                          <span className={`text-[9px] font-bold mt-1 opacity-60 ${isSelected ? "text-white" : isLockedByFull ? "text-rose-300" : "text-slate-400"}`}>
                            {shift.time}
                          </span>
                        </>
                      )}
