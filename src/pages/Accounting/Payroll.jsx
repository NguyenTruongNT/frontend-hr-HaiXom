      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <div
              className={`flex items-center gap-1 px-3 py-1 rounded-xl text-[10px] font-black uppercase ${isPrevMonthLocked ? "bg-emerald-100 text-emerald-600" : "bg-amber-100 text-amber-600"}`}
            >
              {isPrevMonthLocked ? (
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-50">
              {data.length > 0 ? (
                data.map((item) => {
                  const netSalary =
                    item.baseSalary + item.allowances - item.deductions;
                  return (
                    <tr
                      key={item.id}
                    ...
                    >
                      ...
                      <td className="px-8 py-5 text-center">
                        <span
                          className={`px-3 py-1 rounded-full text-[10px] font-black uppercase italic ${item.status === "Hoàn tất" ? "bg-emerald-100 text-emerald-600" : "bg-slate-100 text-slate-400"}`}
                        >
                          {item.status}
                        </span>
                      </td>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-[32px] w-full max-w-md shadow-2xl overflow-hidden border border-white">
            ...
            <div className="p-6 space-y-3">
              {detailModal.list.map((info, idx) => (
                ...
                >
                  ...
                  <span
                    className={`font-black ${detailModal.title.includes("Phụ cấp") ? "text-emerald-600" : "text-rose-500"}`}
                  >
                    {detailModal.title.includes("Phụ cấp") ? "+" : "-"}
                    {info.amount.toLocaleString()} đ
                  </span>
                </div>
              ))}
            </div>
            <div className="p-6 bg-slate-50/50 border-t border-slate-100">
              <button
                onClick={() => setDetailModal({ ...detailModal, show: false })}
                className="w-full py-4 bg-slate-800 text-white rounded-2xl font-black text-[10px] uppercase"
              >
                Đóng chi tiết
              </button>
            </div>
  ...
  current: [
    {
      ...
      status: "Chờ chốt",
      allowanceDetail: [
        { type: "Ăn trưa", amount: 500000 },
        { type: "Trách nhiệm", amount: 2000000 },
        { type: "Xăng xe", amount: 1000000 },
      ],
      deductionDetail: [],
    },
    {
    ...
    },
    {
      ...
      deductionDetail: [
        { type: "BHXH", amount: 1200000 },
        { type: "Thuế TNCN", amount: 300000 },
      ],
    },
    {
      id: "NV004",
      ...
      deductions: 200000,
      status: "Chờ chốt",
      allowanceDetail: [{ type: "Chuyên cần", amount: 800000 }],
      deductionDetail: [{ type: "Vi phạm nội quy", amount: 200000 }],
    },
    {
      id: "NV005",
      name: "Hoàng Văn E",
      ...
      deductions: 0,
      status: "Chờ chốt",
      allowanceDetail: [{ type: "Doanh thu", amount: 1500000 }],
      deductionDetail: [],
    },
    {
      id: "NV006",
      ...
      deductions: 4500000,
      status: "Đã chốt",
      allowanceDetail: [{ type: "Doanh thu", amount: 5000000 }],
      deductionDetail: [
        { type: "BHXH", amount: 2000000 },
        { type: "Tạm ứng", amount: 2500000 },
      ],
    },
    {
      id: "NV007",
      ...
      status: "Chờ chốt",
      allowanceDetail: [],
      deductionDetail: [{ type: "Đồng phục", amount: 100000 }],
    },
  ],
};
