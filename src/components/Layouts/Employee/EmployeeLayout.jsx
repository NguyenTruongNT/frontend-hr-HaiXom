...
const EmployeeLayout = () => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        // CẬP NHẬT: Gọi getMe để lấy đầy đủ thông tin Employee + Branch + Position
        const response = await authApi.getMe();
        setData(response);
      } catch (error) {
        console.error("Lỗi fetch data:", error);
        // Fallback data khớp với cấu trúc CSDL mới
        setData({
          employee: {
            full_name: "Nguyễn Văn A",
            branch: { name: "Cơ sở Thủy Lợi" },
            avatar: "NT",
          },
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="hidden md:flex bg-white border-b p-4 px-8 justify-between items-center sticky top-0 z-30">
          <div>
            ...
            {/* Lưu ý tại Topbar Desktop, sửa hiển thị tên: */}
